...
def write_tables(document: Document, df) -> Document:
    ...
    for line in range(number_lines):
        ...
        for column in range(number_columns):
            try:
                ...
                if df.columns[column] in ['Unit price', 'Amount']:
                    text = format_number(text)
                    print(f"{df.columns[column]}: {text}")
    ...
    amount_list = []
    for number in df['Amount'].dropna():
        try:
    ...
    amount_total_as_string = format_number(str(amount_total))
